...
# ─────────────────────────────────────────────────────────────────────────────
# Test Group 4: Old TTL entries removed (STACK_TTL_SECONDS=1)
# ─────────────────────────────────────────────────────────────────────────────
echo "--- Test Group 4: Old TTL entries removed with short TTL ---"
...
result=$(BATON_ROOT="$TEST_ROOT" STACK_TTL_SECONDS=1 bash -c "
  source '$STATE_MANAGER' 2>/dev/null
  STACK_TTL_SECONDS=1 prune_stale_stack_entries '$STACK_FILE'
" 2>/dev/null)

assert_eq "4a: 2 stale TTL entries (TTL=1s) → returns 2" "2" "$result"
...
FRESH=$(now_ts)
# Use an entry that is 5 seconds old — should survive TTL=7200 but not TTL=1
OLD=$(old_ts_1s)
printf '%s|claude-baton:worker-agent\n' "$OLD" >> "$STACK_FILE"
printf '%s|claude-baton:qa-unit\n' "$FRESH" >> "$STACK_FILE"

# With default TTL (7200s) — both should be kept (5s old < 7200s)
result_default=$(BATON_ROOT="$TEST_ROOT" bash -c "
  source '$STATE_MANAGER' 2>/dev/null
  prune_stale_stack_entries '$STACK_FILE'
" 2>/dev/null)

assert_eq "6a: default TTL=7200 — 5s-old entry kept (returns 0)" "0" "$result_default"

remaining=$(wc -l < "$STACK_FILE" | tr -d ' ')
assert_eq "6b: default TTL — both entries still in file" "2" "$remaining"

# Now with TTL=1 — the 5s-old entry should be pruned, fresh stays
result_short=$(BATON_ROOT="$TEST_ROOT" bash -c "
  source '$STATE_MANAGER' 2>/dev/null
  STACK_TTL_SECONDS=1 prune_stale_stack_entries '$STACK_FILE'
" 2>/dev/null)

assert_eq "6c: TTL=1 — 5s-old entry removed (returns 1)" "1" "$result_short"

remaining=$(wc -l < "$STACK_FILE" | tr -d ' ')
assert_eq "6d: TTL=1 — only 1 fresh entry remains" "1" "$remaining"
